...
PAGES = ["📋 Shortlist", "🔍 Player Analysis", "🏟️ Team Analysis", "⚖️ Compare", "🧠 Smart Query", "🛠️ Query Debug"]
TEAM_METRICS = {
...
<div class="hero">
    <h1>GRIDIRONIQ</h1>
    <p>Preserved real-data dashboard with query-tailored pages and LLM-ready planning/explanations.</p>
</div>
""",
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.markdown(
        '<div class="query-help"><b>Free-text query is enabled.</b><br>Try: <i>show undervalued WRs under $12M</i>, <i>show Chiefs roster</i>, <i>player profile for K.Murray</i>.</div>',
        unsafe_allow_html=True,
    )
    if "user_query_v12" not in st.session_state:
        st.session_state["user_query_v12"] = "Show undervalued RB under $10M"
    user_query = st.text_input("Ask anything about players, contracts, teams, or charts", value=st.session_state["user_query_v12"], placeholder="e.g. show undervalued WRs under $12M")
    for label in ["Show undervalued QBs", "Compare budget WRs", "Plot WR value chart", "Show safe RBs under $15M", "Player profile for K.Murray"]:
...
if page == "📋 Shortlist":
    ...
    if focus_player_row is None:
        ...
        all_options = (results if not results.empty else df)["player_name"].tolist()
        ...
        player_source = results if not results.empty else df
        player_row = player_source[player_source["player_name"] == player_name].iloc[0]
    ...
    if not team_available:
        ...
        with c2:
            ...
            with metric_title_col:
                st.markdown("##### Roster detail chart")
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("#### Dashboard Plan")
        st.json(plan)
    with c2:
        st.markdown("#### Final Filters")
        st.json(final_filters)
    st.markdown("#### Dataset counts")
    st.json({"full_dataset_rows": int(len(df)), "filtered_rows": int(len(results)), "displayed_rows": int(len(display_df)), "unique_players_full": int(df["player_name"].nunique())})
    st.markdown("#### Current selection")
    st.json({"selected_player_name": st.session_state.get("selected_player_name"), "selected_team_name": st.session_state.get("selected_team_name"), "active_page": st.session_state.get("active_page")})

st.markdown(
    """
<div class="footer">
    🏈 GRIDIRONIQ preserved the original real-data views and only changed the requested behavior.<br>
    Core inputs still come from your scored file, season dataset, contracts file, and optional team/headshot outputs.
</div>
""",
    unsafe_allow_html=True,
)
